...
- breakout_bd breakout + 시장 폭(브레드스) 보조필터 (2026-09 검증 통과, 권장 임계 40%):
              전종목 '자기 200일선 위' 비율이 임계 미만인 날은 진입 금지.
              40% 기준 거래당 +0.212%->+0.626%(3배), MDD -38.6%->-25.0%,
              하락장 21-22 누적 -30.9% -> 거래 0건(완전 관망).
              견고성: 30/40/50% 세 임계 전부 모든 구간에서 기존을 이기고, MDD는 단조
              개선(-25.0/-25.0/-20.8), 하락장 무력화도 임계 무관하게 일관.
              (미장 110종목에서는 승자 편향으로 비율 중앙값이 74%라 판정 불가였고 임계를
              올리자 55/60/65%에서 비단조로 요동쳐 폐기. 전종목 2228개 중앙값 34%에서만
              내부 붕괴가 실제로 측정된다 -> 브레드스는 큐레이션 없는 유니버스가 필수.)
              주의: 누적은 거래 빈도에 좌우되는 복리 산물이라 판단은 거래당·MDD로 할 것.
              잔여 생존편향(과거 상폐 종목 DB 부재)은 남아 있다.
